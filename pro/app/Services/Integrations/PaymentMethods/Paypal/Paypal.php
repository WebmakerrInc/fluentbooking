...
class Paypal extends BasePaymentMethod
{
    ...
    public function makePayment($orderItem, $calendarEvent, $order)
    {
        ...
        $paymentArgs = wp_parse_args($paymentArgs, $this->getCartSummery($items, $discountTotal));
    ...
    private function getCartSummery($items, $discountTotal)
    {
        $paypalArgs = [];
        ...
        if ($discountTotal) {
            $paypalArgs['discount_amount_cart'] = round($discountTotal / 100, 2);
        }
